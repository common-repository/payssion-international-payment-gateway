...
class WC_Gateway_Payssion extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = __( $this->is_channel ? '' : 'Payssion provides a global payment solution. Please keep this method as disabled and just activate these payment methods you need.', 'woocommerce' );
		$this->supports           = array(
			'products'
		);
		...
		//add_action('woocommerce_order_status_completed', [ $this, 'shipOrder' ]);
...
// 	}
	...
	public function init_form_fields() {
		...
		if ($this->is_channel) {
		    ...
		    $debug_description = null;
		    if ( version_compare( WC()->version, '8.6', '<' ) ) {
		        // For WooCommerce versions before 8.6
		        $log_file_path = wc_get_log_file_path( 'Payssion' );
		        $debug_description = sprintf( __( 'Log Payssion events, inside <code>%s</code>', 'woocommerce' ), $log_file_path);
		    } else {
		        // For WooCommerce 8.6 and later
		        //$log_handler = wc_get_logger()->get_handler( 'file' );
		        //$log_file_path = $log_handler->get_log_file_path( 'Payssion' );
		        $debug_description = 'Log Payssion events';
		    }
			$this->form_fields = array(
					'testmode' => array(
...
// 		);
	...
	function generate_payssion_form( $order_id ) {
		...
		$url = $request->get_request_url($order, $this->testmode );
